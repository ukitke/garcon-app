...
resource "aws_lb_target_group" "backend_http" {
  ...
  tags = {
    Name = "${var.project_name}-backend-http-tg"
  }
}

resource "aws_lb_target_group" "backend_websocket" {
  name     = "${var.project_name}-backend-ws"
  port     = 3001
  protocol = "HTTP"
  vpc_id   = aws_vpc.main.id
  target_type = "ip"
  ...
  tags = {
    Name = "${var.project_name}-backend-ws-tg"
  }
}
...
resource "aws_lb_listener_rule" "websocket" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.backend_websocket.arn
  }
